...
class SlackSearchDataaccess:
    ...
    def search(self, slack_user_id_list, channel_id_list, search_val_list, search_type, is_contains_reply, from_date, to_date) -> pd.DataFrame:
        ...
        params = []
        # 投稿内容
        sql_post, params_post = _create_sql_search(slack_user_id_list, channel_id_list, search_val_list, search_type, from_date, to_date, is_post=True)
        sql += sql_post
        params.extend(params_post)
        # 返信内容
        if is_contains_reply:
            sql += '\n    UNION ALL\n'
            sql_reply, params_reply = _create_sql_search(slack_user_id_list, channel_id_list, search_val_list, search_type, from_date, to_date, is_post=False)
            sql += sql_reply
...
def _create_sql_search(slack_user_id_list, channel_id_list, search_val_list, search_type, from_date, to_date, is_post=True):
    """
    検索用SQL組み立て
    ...
    """
    alies = 'history' if is_post else 'reply'
    ...
    if slack_user_id_list:
        ...
        sql += f'\n      AND {alies}.{prefix}_slack_user_id IN ({", ".join(placeholder_list)}) '
        params.extend(val_list)
    # チャンネル
    if channel_id_list:
        placeholder_list = ['?' for _ in channel_id_list]
        val_list = [v for v in channel_id_list]
        sql += f'\n      AND channel.channel_id IN ({", ".join(placeholder_list)}) '
        params.extend(val_list)
    # 期間
    if from_date:
        sql += f'\n      AND {alies}.{prefix}_date >= ?'
    ...
    if search_val_list:
        ...
        for search_val in search_val_list:
            list_.append(f"{alies}.{prefix}_message LIKE ?")
